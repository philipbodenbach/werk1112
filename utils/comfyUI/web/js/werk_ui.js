...
function modelValues(discovery, requireAvailable = true) {
...
function updateImageModelsNode(node, discovery) {
    ...
    const values = modelValues(discovery, requireAvailable);
...
app.registerExtension({
    ...
    loadedGraphNode(node) {
        const className = nodeClass(node);
        if (className === CONNECTION_CLASS || className === IMAGE_MODELS_CLASS) {
            fitWerkNode(node);
