...
export function chatOptionsProperty(vision: boolean): INodeProperties {
	return collectionProperty('options', vision ? 'Vision Options' : 'Chat Options', [
		...
		...(vision ? [optionsProperty('imageDetail', 'Image Detail', ['auto', 'low', 'high'])] : [
			...
			{ ...optionsProperty('omlxReasoningEffort', 'oMLX Reasoning Effort', ['inherit', 'low', 'high', 'max']), description: 'Native reasoning effort for compatible templates, including GLM. Low does not disable reasoning.' },
			...
			{ ...triProperty('omlxNgramOffload', 'oMLX N-Gram Offload'), options: [{ name: 'Server Default', value: 'inherit' }, { name: 'Auto', value: 'auto' }, { name: 'Manual Cache Limit', value: 'enabled' }, { name: 'Disabled (Resident Tables)', value: 'disabled' }], description: 'Auto adapts to memory and row demand. Manual uses the MiB limit. Independent of MoE experts; requires a verified architecture adapter.' },
			{ ...numberProperty('omlxNgramCacheMb', 'oMLX N-Gram Cache (MiB)', 1024, 1), typeOptions: { minValue: 1, maxValue: 1048576, numberPrecision: 0 }, displayOptions: { show: { omlxNgramOffload: ['enabled'] } }, description: 'N-gram row cache limit; shares available system memory with experts, dense weights and KV' },
			{ ...triProperty('omlxExpertOffload', 'oMLX Expert Offload'), options: [{ name: 'Server Default (Auto Unless Configured)', value: 'inherit' }, { name: 'Manual Cache Limit', value: 'enabled' }, { name: 'Disabled (Native Loading)', value: 'disabled' }], description: 'Server Default omits the override and uses automatic hardware/model sizing on supported Werk/oMLX setups unless the server has an explicit budget. Manual preserves small-machine budgets. This controls expert weights, not the prompt/KV cache.' },
			{ ...numberProperty('omlxExpertCacheMb', 'oMLX Expert Cache (MiB)', 8192, 1), typeOptions: { minValue: 1, maxValue: 1048576, numberPrecision: 0 }, displayOptions: { show: { omlxExpertOffload: ['enabled'] } }, description: 'Explicit expert cache ceiling in MiB; used only with Manual Cache Limit. The memory guard may reduce residency. Other weights, KV and temporary buffers need additional memory. Uncached experts remain checkpoint-backed.' },
		]),
	]);
}
