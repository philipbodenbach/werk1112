...
export class WerkText implements INodeType {
	description = nodeDescription('werkText', 'WERK Text (Beta)', [
		...
		{ displayName: 'Messages', name: 'messages', type: 'fixedCollection', default: { message: [{ role: 'user', content: '' }] }, typeOptions: { multipleValues: true, sortable: true }, options: [{ name: 'message', displayName: 'Message', values: [
			optionsProperty('role', 'Role', ['system', 'developer', 'user', 'assistant', 'tool'], 'user'),
		...
		{ name: 'conversationHistory', displayName: 'Conversation History (JSON)', type: 'json', default: '[]', description: 'Previous OpenAI messages, prepended to Messages. Connect a prior Text output using {{ $json.conversation }}. Includes tool calls/results; each workflow item keeps its own history. Native prompt-cache reuse is managed by Werk.' },
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		return executeItems(this, async (client, index) => {
			...
			const request = buildTextRequest(this.getNodeParameter('model', index, undefined, { extractValue: true }), this.getNodeParameter('messages', index), this.getNodeParameter('options', index, {}), this.getNodeParameter('conversationHistory', index, '[]'));
