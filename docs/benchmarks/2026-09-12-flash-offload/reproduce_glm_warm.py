#!/usr/bin/env python3
"""Three identical Rust prompts in a growing GLM conversation; optional label.

Uses temperature zero, Auto expert memory, reasoning low and persistence.
...
out=ROOT/'docs/benchmarks/2026-09-12-flash-offload'/('glm-warm-'+(sys.argv[1] if len(sys.argv)>1 else 'baseline'))
...
env['WERK_OMLX_REASONING_EFFORT']='low'
...
reference=json.loads((ROOT/'docs/benchmarks/2026-09-12-flash-offload/glm-warm-after.json').read_text()) if fixed_history else None
binary=os.environ.get('WERK_FLASH_BIN',str(ROOT/'target/release/werk'))
report={'model':model,'binary':binary,'same_context':same_context,'persistence':persistence,'fixed_history':fixed_history,'expert_budget':env['WERK_OMLX_EXPERT_CACHE_MB'],'reasoning_effort':env.get('WERK_OMLX_REASONING_EFFORT'),'samples':[],'status':[]};stop=threading.Event();monitor=None
...
with out.with_suffix('.log').open('w') as log:
 ...
 try:
  ...
  messages=[]
  prompts=['Write me a single sentence about the programming language Rust.']*3
  for turn,prompt in enumerate(prompts):
   ...
   result=chat.request_chat(url+'/v1/chat/completions',key,payload,180,300);result['before']=before;result['after']=status();result['status_range']=[start_index,len(report['status'])];report['samples'].append(result);result['correct']='rust' in result['answer'].lower() and result['finish_reason']=='stop';print(json.dumps(result),flush=True)
  ...
  for sample,phase in zip(report['samples'],phases):
   ...
   # Only snapshots after a decode callback for this request. GLM has 42
   # streamed expert layers; counter reads can occur partway through a token.
   ...
   if len(states)>1:
    first,last=states[0],states[-1];tokens=(last['forward_calls']-first['forward_calls'])/42
