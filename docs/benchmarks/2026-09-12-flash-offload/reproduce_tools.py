"""Public Qwen tool round-trip and ordinary text with an attached tool catalog."""
import json
import os
...
OUTPUT = Path(os.environ.get('WERK_FLASH_TOOL_REPORT', '/tmp/werk-qwen-tools.json'))
...
def main():
    ...
    env = dict(os.environ, WERK_API_KEY=key, WERK_OMLX_EXPERT_CACHE_MB='auto',
               WERK_OMLX_NGRAM_CACHE_MB='1024', WERK_OMLX_THINKING='0')
    report = {'model': MODEL, 'expert_cache_mb': 'auto', 'ngram_cache_mb': 1024, 'samples': []}

    def request(messages, stream):
        payload = dict(model=MODEL, messages=messages, tools=TOOLS, tool_choice='auto',
                       stream=stream, temperature=0, max_tokens=128)
    ...
    with OUTPUT.with_suffix('.log').open('w') as log:
        ...
        try:
            ...
            message, finish = request([{'role': 'user', 'content':
                'Write me a single sentence about the programming language Rust. Do not use tools.'}], True)
