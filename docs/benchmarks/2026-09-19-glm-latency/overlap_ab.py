...
def main():
    ...
    parser.add_argument('--model', default='Vontra/GLM-5.3-Flash-MLX-oQ2-MTP')
    parser.add_argument('--cache-mb', default='22528')
    args = parser.parse_args()
    args.output.mkdir(parents=True, exist_ok=True)
    env = {k: v for k, v in os.environ.items() if not k.startswith(('WERK_OMLX_', 'OMLX_'))}
    env.update(WERK_OMLX_EXPERT_CACHE_MB=args.cache_mb, WERK_OMLX_NGRAM_CACHE_MB='auto',
               WERK_OMLX_THINKING='0', WERK_OMLX_REASONING_EFFORT='low', WERK_OMLX_EXPERT_EXECUTION='grouped')
    rows = []
    for index, label in enumerate(('before', 'after', 'after', 'before')):
        ...
        row = dict(model=args.model, cache_mb=args.cache_mb, label=label, wall_seconds=elapsed, eval_rate=metric('eval rate'),
