#!/usr/bin/env python3
"""Two identical token prompts on one GLM worker; use --same-context --no-persistence.
...
out=ROOT/'docs/benchmarks/2026-09-19-glm-latency/warm-same-context'
...
with out.with_suffix('.log').open('w') as log:
 ...
 try:
  ...
  messages=[]
  prompts=['Explain what a token is in three sentences.']*2
  for turn,prompt in enumerate(prompts):
   ...
   result=chat.request_chat(url+'/v1/chat/completions',key,payload,180,300);result['before']=before;result['after']=status();result['status_range']=[start_index,len(report['status'])];report['samples'].append(result);result['completed']=result['finish_reason']=='stop' and not result['error'];print(json.dumps(result),flush=True)
