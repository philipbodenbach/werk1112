...
def main():
    ...
    parser.add_argument('--tuning', action='store_true', help='Two identical 128-token requests without history; one process')
    parser.add_argument('--chat-smoke', action='store_true', help='Three user-style Rust prompts with persistence and default sampling')
    parser.add_argument('--generation-threads', type=int)
    parser.add_argument('--prompt-threads', type=int)
    parser.add_argument('--expert-cpu-layers', type=int)

    args = parser.parse_args()
    if args.generation_threads is not None and not 1 <= args.generation_threads <= 48:
        parser.error('--generation-threads must be 1..48')
    if args.prompt_threads is not None and not 1 <= args.prompt_threads <= 48:
        parser.error('--prompt-threads must be 1..48')
    if args.expert_cpu_layers is not None and not 0 <= args.expert_cpu_layers <= 43:
        parser.error('--expert-cpu-layers must be 0..43')
    if args.tuning and args.chat_smoke:
        parser.error('--tuning and --chat-smoke are mutually exclusive')
    if args.chat_smoke:
        args.phases = 1
        PROMPTS[:] = [('Write me a sentence about the programming language rust.\n' * 3) + '/exit\n']
    if args.tuning:
        args.phases = 1
        prompt = 'Explain how Rust ownership and borrowing work. Give a detailed practical example and discuss the tradeoffs.'
        PROMPTS[:] = [prompt + '\n' + prompt + '\n/exit\n']
    ...
    placement = '--cpu-moe' if args.expert_cpu_layers is None else f'--n-cpu-moe {args.expert_cpu_layers}'
    env['WERK_LLAMA_ARGS'] = placement + ' --reasoning off --verbosity 4'
    ...
    if args.tuning:
        i = command.index('--persistence')
        command[i:i + 3] = ['--no-history']
        command[command.index('--max-tokens') + 1] = '128'
    if args.chat_smoke:
        command[command.index('--max-tokens') + 1] = '128'
        i = command.index('--temperature')
        command[i:i + 2] = []
    for option, value in [('--threads', args.generation_threads), ('--threads-batch', args.prompt_threads)]:
        if value is not None:
            command[1:1] = [option, str(value)]
    # Refuse a concurrent large inference process rather than evicting its weights.
    running = subprocess.run(['pgrep', '-x', 'llama-server'], capture_output=True, text=True)
    if running.returncode == 0:
        raise SystemExit('An existing llama-server is running; stop that chat before this isolated test.')
    ...
    metadata = {
        ...
        'phase_timeout_seconds': args.phase_timeout, 'cpu_thread_policy': {'generation': args.generation_threads, 'prompt': args.prompt_threads, 'unset': 'native default'},
