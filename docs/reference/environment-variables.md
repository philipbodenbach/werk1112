...
| `WERK_OMLX_TEXT_DECODE` | Single-token expert output assembly for the verified GLM `glm5_next` and Qwen `qwen4_exp` text offload adapters: `direct` (default) assembles evaluated expert rows in route order; `legacy` uses the original gather/scatter implementation. Prefill, quantization, active-weight lifetimes and cache budgets remain the same. Restart Werk when changing the setting. DeepSeek and other backends do not use it. |
| `WERK_OMLX_GLM_PROFILE` | Optional GLM `glm5_next` expert-layer counters: `1` enables bounded cumulative per-layer statistics in the private worker status and verbose generation diagnostics; `0` (default) disables them. Measures existing counters and wall time, with overlapping phases and logical reads that may hit the OS file cache. Profiling adds overhead; leave disabled for performance comparisons. Qwen and DeepSeek do not use it. |
| `WERK_OMLX_NGRAM_CACHE_MB` | Separate N-gram cache cap in MiB for verified text offload adapters. Unset or `0` keeps tables resident. Explicit `auto` starts with at most 64 MiB for Qwen `qwen4_exp` PLE rows and grows at prefill/request boundaries only after demand eviction. The hardware ceiling uses at most one eighth of shared weight-cache room when experts are streamed; native expert weights are otherwise charged as base memory. Pressure can shrink the row cache. There is no fixed 1-GiB Auto ceiling; `0` keeps tables resident. A positive integer requires actual supported tables. Both caches share native memory admission, so their effective limits may shrink. API `werk.omlx.ngram_cache_mb` and ComfyUI/n8n text controls can override it. |
